...
// TODO: Swingのtext.Segmentの概念を調べる必要がある。
...
public class SyntaxUtilities {
	...
	/**
	 * Checks if a sub region of a <code>Segment</code> is equal to a character
	 * array.
	 * 
	 * @param ignoreCase
	 *            True if case should be ignored, false otherwise
	...
	public static SyntaxStyle[] getDefaultSyntaxStyles() {
		// テキストエリアに表示される文字列は、トークンとして分けることが出来る。これは自然言語でいうところの単語に分ける
		// それで、各トークンスタイルを色、ボールド、イタリックで作成しておいて、トークンの属性として保持させることでスタイル付き文字列となる。
		// これは、AttributedStringとTextLayoutで保持できるのでは？
	...
	public static int paintSyntaxLine(Segment line, Token tokens, SyntaxStyle[] styles, TabExpander expander,
		...
		for (;;) {
			...
			line.count = length; // トークンの長さがline.count?
